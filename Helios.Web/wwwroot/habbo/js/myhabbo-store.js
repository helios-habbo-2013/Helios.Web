var WebStore = {
  ...
  _loadMainContent: function(tab) {
    if (tab == "webstore-inventory") {
      ...
      new Ajax.Request(
        lolcats + "myhabbo/store/main.cfm", {
          method: "post",
      ...
      try {
        if (el.id == "webstore-purchase-disabled") {
          ...
          if (!WebStore.backgroundPreviewWarning) {
            ...
            new Ajax.Request(lolcats + "myhabbo/store/background_warning.cfm", {
                method: "post",
  ...
  _loadSubCategory: function(categoryId, subCategoryId, productId) {
    var query = "categoryId=" + encodeURIComponent(categoryId);
    if (subCategoryId) {
      query += "&subCategoryId=" + encodeURIComponent(subCategoryId);
    }
    new Ajax.Request(
      lolcats + "myhabbo/store/items.cfm", {
  ...
  _loadPreview: function(productId) {
    ...
    WebStore._showPreview();
    new Ajax.Request(
      lolcats + "myhabbo/store/preview.cfm", {
  ...
  _purchaseConfirm: function(productId) {
    ...
    new Ajax.Request(lolcats + "myhabbo/store/purchase_confirm.cfm", { 
      method: "post", parameters: { "productId" : productId, "subCategoryId" :  WebStore.selectedSubCategory }, 
      onComplete: function(req, json) {
        if (WebStore._checkResponse(req.responseText)) {
  ...
  _purchase: function(productId, dialog) {
    ...
    new Ajax.Request(lolcats + "myhabbo/store/purchase_" + type + ".cfm", { 
      method: "post", parameters: "task=purchase&selectedId=" + encodeURIComponent(productId), onComplete: function(req, json) {
        if (WebStore._checkResponse(req.responseText)) {
...
WebStore.MenuBar = {
  ...
  selectedSubcategory: null, 
    ...
    WebStore.MenuBar.selectedMainCategory = document.getElementsByClassName("webstore-selected-main", $("webstore-categories")).first();
    if (WebStore.MenuBar.selectedMainCategory) WebStore.selectedCategory = WebStore.MenuBar.selectedMainCategory.id.split("-")[1];
    WebStore.MenuBar.selectedSubcategory = document.getElementsByClassName("subcategory-selected", WebStore.MenuBar.selectedMainCategory).first();
    ...
    changeMainCategory: function(mainCategoryId, mainCategoryEl, mainCategoryType) {
      ...
      if (switched) {
        ...
        WebStore._setDialogSize(mainCategoryType, function() {
          var subcategoryEl = document.getElementsByClassName("subcategory", mainCategoryEl).first();
  ...
  _hideSubcategories: function(){
    document.getElementsByClassName("purchase-subcategory-list").each(function (el) { Element.hide(el); });
  },
...
WebStore.Inventory = {
  ...
  load: function() {
    new Ajax.Request(
      lolcats + "myhabbo/store/inventory.cfm", {
  ...
  loadCategory: function(type) {
    if (!WebStore.Inventory.loadingCategory) {
      ...
      new Ajax.Request(
        lolcats + "myhabbo/store/inventory_items.cfm", {
          method: "post", parameters: "type=" + encodeURIComponent(type),
          onComplete: function(req, json) {
            if (WebStore._checkResponse(req.responseText)) {
  ...
  _loadPreview: function(itemId, type, privileged) {
    ...
    WebStore.Inventory._showPreview();
    var qs = "itemId=" + itemId + "&type=" + encodeURIComponent(type);
    if (type == "widgets") {
      qs += "&privileged=" + privileged;
    }
    new Ajax.Request(
      lolcats + "myhabbo/store/inventory_preview.cfm", {
...
WebStore.StickerEditor = {
  ...
  open: function() {
    new Ajax.Request(
      lolcats + "myhabbo/stickereditor/stickereditor_flash", {
  ...
  preview: function(figureParams) {
    ...
    new Ajax.Request(lolcats + "myhabbo/stickereditor/preview", {
  ...
  _purchase: function(imageUrl) {
    new Ajax.Request(lolcats + "myhabbo/store/purchase_avatarsticker", {
      method: "post", parameters: "stickerimage=" + encodeURIComponent(imageUrl),
      onComplete: function(req, json) {
        if (WebStore._checkResponse(req.responseText)) {
          if (req.responseText.strip() != "OK") {
...
WebStore.NoteEditor = {
  ...
  _loadEditor: function(noteParams, backFromPreview) {
    ...
    new Ajax.Request(lolcats + "myhabbo/noteeditor/editor.cfm", {
            ...
            if (WebStore._checkResponse(req.responseText)) {
              ...
              if ($("webstore-notes-continue")) { Event.observe($("webstore-notes-continue"), "click", function(e) {
                ...
                if (!el.className || !Element.hasClassName(el, "disabled-button")) {
                  ...
                  Dialog.setAsWaitDialog(WebStore.NoteEditor.dialog);
                  new Ajax.Request(lolcats + "myhabbo/noteeditor/preview.cfm", {
                    method: "post", parameters: WebStore.NoteEditor.noteParams, onComplete: function(req, json) {
                      if (WebStore._checkResponse(req.responseText)) {
  ...
  _place: function() {
    if (WebStore.NoteEditor.noteParams) {
      new Ajax.Request(lolcats + "myhabbo/noteeditor/place.cfm", {
        method: "post", parameters: WebStore.NoteEditor.noteParams, onComplete: function(req, json) {
          if (WebStore._checkResponse(req.responseText)) {
            if (req.responseText.strip() == "BACK") {
              WebStore.NoteEditor._loadEditor(WebStore.NoteEditor.noteParams, true);
  ...
  _saveChanges: function() {
    if (WebStore.NoteEditor.noteParams) {
      new Ajax.Request(lolcats + "myhabbo/noteeditor/save", {
        method: "post", parameters: WebStore.NoteEditor.noteParams, onComplete: function(req, json) {
          if (WebStore._checkResponse(req.responseText)) {
            if (req.responseText.strip() == "BACK") {
              WebStore.NoteEditor._loadEditor(WebStore.NoteEditor.noteParams, true);
